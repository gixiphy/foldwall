...
| # | 項目 | 結果 |
| --- | --- | --- |
| 8 | `xcodebuild -scheme Foldwall -destination 'platform=macOS' test` | **綠**，235 測試（1 個視覺樣張預設跳過） |
...
安裝檔：`dist/Foldwall-0.5.1-arm64.dmg`（拖進 Applications 後從那裡啟動）
...
| 28 | 從網址下載 | **已於 2026-08-25 實測通過**：用 app 完全相同的參數對直連 mp4 下載成功（6.5MB，`Destination:` 輸出格式與解析相符），存進 `~/Movies/Foldwall` 後自動進影片池。**待人工**：在設定頁貼網址按下載。注意有些站擋在 Cloudflare 後面會失敗，Foldwall **刻意不加**偽裝參數 |
...
| 29 | 播完接下一支（桌面視窗） | **待人工**：影片來源放幾支**短片**（30 秒以內，不然要等很久）。播放方式選「全部循環」→ 一支播完應自動接下一支，log 出現 `播畢` 接著 `桌面視窗開始播`，**中間不該黑畫面**；選「隨機播放」→ 下一支不該是剛播完那支；選「單片循環」→ 應無縫接回開頭、不換片。雙螢時兩台不該同時播同一支 |
| 30 | 手動下一片 | **待人工**：選單列「下一片影片」（⇧⌘N）三種模式下都要立刻換片。按「下一張」則**只**換蒙太奇、不動影片。改播放方式時正在播的那支會重播一次（循環方式是建 player 當下決定的） |
| 31 | 播完接下一支（extension） | **待人工**：系統設定 → 桌布 → **Shuffle All** → *Change Video* 選 **After Each Video**，確認一支播完會換下一支且**換片是無縫的**（走 loop boundary 預載）。選固定某一支時不該換。app 的「下一片影片」在 Shuffle All 下應也能跳片 |
| 32 | 影片縮放 | **待人工**：影片來源放一支**直式**、一支**橫式**的片。設定 → 影片桌布 → 播放方式 → 縮放（選單列「影片縮放」也一樣）：「填滿螢幕」該裁切、「符合螢幕大小」該留黑邊、「填滿高度」直式片該留**左右**黑邊而橫式片被裁左右、「填滿寬度」反過來——**四種都不准把影片拉扁**。改的當下要立刻生效且不重播（log 出現 `影片縮放改為`；長寬比到手後還會有一行 `影片長寬比 … → fill/fit`）。「隨機」→ 每支片可能不同，但同一支重播、重開 app 之後仍是同一種。系統 extension 那條也要跟著變（含鎖屏），extension 日誌會出現 `Scale mode →` |
...
| 13 | 輪替涵蓋全庫 | **待人工**：反覆喚醒數次，確認游標持續前進並會繞回開頭，container 始終在 2GB 以內 |
| 7 | 無 Google | 已由自動檢查涵蓋 |

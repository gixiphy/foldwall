...
安裝檔：`dist/Foldwall-0.3.6-arm64.dmg`（拖進 Applications 後從那裡啟動）
...
| 12 | 影片輪替觸發點 | **待人工**：桌布間隔設 5 分鐘，確認影片**不跟著**換；讓螢幕睡著或跑螢保後喚醒，確認換了一批新的 1–3 支（看 log 的 `這輪帶 N 支，游標推到 X／Y`）；30 分鐘內重複喚醒不應重拷 |

...
| **模型不用自己猜** | 問得到清單的引擎（Antigravity、Grok、OpenCode、Codex）在模型欄位旁給下拉選單，直接挑；Claude Code 給 `opus`／`sonnet`／`fable`／`haiku` 這些恆指向當代最新模型的別名。欄位**永遠可以自由輸入**——清單可能過期或列不全，不該因此擋住你想用的模型。 |

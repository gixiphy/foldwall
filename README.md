...
需要 macOS 26 以上、Apple Silicon。介面有繁體中文、簡體中文與英文，跟著系統語言走；其他語言可以在「設定 → 語言」用你本機已登入的 AI CLI（Claude Code、Codex 等）自己翻一份，翻好的檔只留在這台 Mac。
...
介面內建繁體中文、簡體中文與英文，跟著系統語言走。其他語言不由我這邊維護——沒有人能校對——而是在「設定 → 語言」交給**你本機已登入的 AI CLI** 翻一份。

翻譯來源是內建的**英文**（模型英譯外語的品質普遍比中譯外語好，繁中原文一起附上當第二參考），約 400 條字串，40 條一批送出，通常幾分鐘跑完。

| 項 | 說明 |
| --- | --- |
| **支援的 CLI** | Claude Code、Codex CLI、Antigravity、Grok Build、OpenCode。掃描順序是自訂路徑 → `PATH` → 常見安裝位置；沒偵測到的可以直接填執行檔完整路徑。 |
...
每翻完一批就寫一次檔，所以中途按取消或斷在一半，已經翻好的都留著。

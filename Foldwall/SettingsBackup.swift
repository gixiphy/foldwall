...
final class SettingsBackup {
    ...
    var displayPath: String {
        guard let fileURL else { return String(localized: "iCloud Drive 未啟用") }
    ...
    func export(_ snapshot: SettingsSnapshot) -> Bool {
        ...
        do {
            ...
            status = .ok(String(localized: "已備份到 iCloud（\(Self.stamp(snapshot.savedAt))）"))
    ...
    func load() -> SettingsSnapshot? {
        ...
        if !FileManager.default.fileExists(atPath: fileURL.path(percentEncoded: false)) {
            ...
            status = .failed(String(localized: "iCloud 上還沒有備份"))
            return nil
        }
        do {
            ...
            status = .failed(String(localized: "備份是較新的格式（v\(version)），請先更新這台的 Foldwall"))
            return nil
        } catch {
            status = .failed(String(localized: "讀取備份失敗：\(error.localizedDescription)"))
            return nil
        }
    }
    ...
    func didApply(_ snapshot: SettingsSnapshot, quietly: Bool = false) {
        ...
        lastRemoteModified = modificationDate()
        if !quietly {
            status = .ok(String(localized: "已從 iCloud 匯入（\(snapshot.deviceName)・\(Self.stamp(snapshot.savedAt))）"))
        }
        refreshRemoteSummary()
    }
    ...
    func refreshRemoteSummary() {
        ...
        let stamp = Self.stamp(snapshot.savedAt)
        remoteSummary = String(localized: """
            \(snapshot.deviceName)・\(stamp)・\(snapshot.folders.count) \
            個資料夾・\(snapshot.albums.count) 個相簿
            """)
    }
